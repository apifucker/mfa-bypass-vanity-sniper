...
function connectWebSocket() {
  ...
  websocket.onmessage = async (message) => {
    ...
    if (t === "GUILD_UPDATE") {
      ...
      console.log(`\x1b[31mrush wishes you a good flight!\x1b`);
    }

    async function notifyWebhook(find) {
      const requestBody = {
        content: `*⌜ code : ' ${find} ' ⌟* @everyone @here https://tenor.com/view/akame-akame-ga-k-ill-anime-fighting-stance-windy-gif-17468654`
      };
...
async function vanityUpdate(find) {
  try {
    const vanityResponse = await http2Request('PATCH', `/api/v9/guilds/${config.guildId}/vanity-url`, {
      ...headers,
      'X-Discord-MFA-Authorization': rushhhh,
      'Cookie': `__Secure-recent_mfa=${rushhhh}`,
      'Content-Type': 'application/json'
